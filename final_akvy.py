...
s=d["Sales"]
c=d["Customer"]
Modules=["Home","Customers","Category","Products","Customer Bookings","Sales","Customer Reviews"]
...
if selectbox==Modules[0]:
    st.markdown("<h1 style='text-align: center;'>Welcome to AKVY Store</h1>",unsafe_allow_html=True)
    ...
    st.image(image,width=700, use_column_width=400)
    ...
    st.dataframe(d)
    d['Count']=li
    table=pd.pivot_table(d,index='Date',values='Count',aggfunc=np.sum)
    #st.dataframe(table)
    st.markdown("<h2 style='text-align: center;'><b><i>Count of Customers</i></b></h2>",unsafe_allow_html=True)
    st.bar_chart(table)
    ...
    st.markdown("<h1 style='text-align: center;'><b>PRODUCTS</b></h1>",unsafe_allow_html=True)
    image = Image.open('./products.jpeg')
    st.image(image,width=700, use_column_width=200)
    fr=p.find({},{"_id":0})
    ...
    st.markdown("<h1 style='text-align: center;'><b>CUSTOMER BOOKINGS</b></h1>",unsafe_allow_html=True)
    image = Image.open('./cust_bookings.jpeg')
    st.image(image,width=700, use_column_width=200)
    fr = cb.find({},{"_id":0})
    st.dataframe(fr)
elif selectbox==Modules[5]:
    st.markdown("<h1 style='text-align: center;'><b>SALES</b></h1>",unsafe_allow_html=True)
...
elif selectbox==Modules[6]:
    st.markdown("<h1 style='text-align: center;'><b>CUSTOMER REVIEWS</b></h1>",unsafe_allow_html=True)
    ff=p.find({})
    for i in ff:
        ...
        if len(i['Review'])==0:
            ...
            for j in range (len(i['Review'])):
                st.write(i['Reviewer Name'][j]+" : "+i['Review'][j])
        stri=''
        for i in range (int(i['Ratings'])):
            stri=stri+':star: '
        st.write('Ratings: ',stri)
        st.write('----------------------------------------------------------')
